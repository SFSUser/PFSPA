...
function trackEvent(cat1, cat2, cat3){
    if(typeof(ga_pre_cat) != 'undefined'){
        //ga_pre_cat = "";
        cat1 =  ga_pre_cat + " > " + cat1;
    }
...
(function($) {
    $.fn.tabber = function (e) {
        ...
        $(this).children("ul:first").find("li:first").addClass("active");
        $(this).children("ul:first").children("li").click(function () {
            if(e.onchange !== undefined){
                var cur_tab = $(this).parent().find(".active");
                if(!e.onchange(cur_tab)) return;
            }
            $(this).parent().find("li").removeClass("active");
            container.children("div").hide();
            $(this).addClass("active");
            container.find("#tab_" + $(this).attr("tab")).show();
        });
    };
