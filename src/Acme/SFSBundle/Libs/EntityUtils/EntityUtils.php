 ...
 */
class EntityUtils {
    public static function nativeQuery($doctrine, $query, $values) {
        //$doctrine = $this->getDoctrine();
        $manager = $doctrine->getManager();
        $connection = $manager->getConnection();
        $statement = $connection->prepare($query);
        foreach ($values as $x => $y) {
            $statement->bindValue($x, $y);
        }
        $statement->execute();
    ...
    public static function entityObjectFormat($doctrine, &$value) {
        if ($value === null)
            return null;
        $value->formatted = self::entityObjectToArray($doctrine, $value);
        return $value;
    }

    public static function entityObjectsFormat($doctrine, &$array) {
    ...
    public static function entityObjectsToArray($doctrine, $array, $append_element = false) {
        $elements = array();
        foreach ($array as $value) {
            $elements[] = self::entityObjectToArray($doctrine, $value, $append_element);
        }
        return $elements;
    }
     ...
     */
    public static function entityObjectToArray($doctrine, $element, $append_element = false) {
            ...
            switch ($column_type) {
                ...
                case "integer":
                    ...
                    if(method_exists($element, "getSearchData")){
                        $search_data = $element->getSearchData();
                        $sdv = Utils::getParam($search_data, $column_name, null);
                        $ref_name = Utils::getParam($sdv, $column_value, $ref_name);
                    }
